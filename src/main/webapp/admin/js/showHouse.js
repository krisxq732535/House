$(function () {
    $('#dg').datagrid({
        url:'HouseController/getCheckHouse.do',
        pagination:true,
        pageSize:10,
        pageList:[10,20,30],
        columns:[[
           /* {field:'opt',checkbox:'true',width:100},*/
            ...
            {field:'d',title:'操作',width:100,
                formatter: function(value,row,index){
                    return "<a href='javascript:checks("+row.id+");'>审核</a>"
                }
            }
        ]]
    });
...
function checks(id) {
        $.post("HouseController/updatePass.do",{"id":id},function (data) {
                if(data.result>0){
                   $("#dg").datagrid("reload");
                }else {
                    $.messager.alert("error","审核失败");
                }
        },"json")
}/*
/!*区域的批量删除*!/
