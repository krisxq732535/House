$(function () {
    $('#dg').datagrid({
        url:'street/selectStreetByPage.do',
        toolbar:'#tb',//绑定工具栏
        pagination:true,
        pageSize:3,
        pageList:[3,6,9],
...
function del() {
    ...
    $.messager.confirm('删除操作', '确定需要删除吗？', function(r){
        if (r){
            ...
            var $opts=$("#dg").datagrid("getSelections");
            $.post('street/deleteStreet.do',{"id":$opts[0].id},
                function (data) {
                    if(data.result==1){
                        $("#dg").datagrid("reload");
    ...
    if($opts.length>=1){
        $.messager.confirm('删除操作', '确定需要删除吗？', function(r){
            if (r){
                ...
                $.post('street/deleteMoreStreet.do',{"id":ids},
                    function (data) {
                        if(data.result>=1){
                            $.messager.alert("Warning","删除成功");
                            $("#dg").datagrid("reload");
