$(function () {
    $('#dg').datagrid({
        url:'HouseController/getCheckHouse2.do',
        pagination:true,
        pageSize:10,
        pageList:[10,20,30],
        columns:[[
            ...
            {field:'floorage',title:'范围大小',width:100},
            {field:'contact',title:'联系方式',width:100},
            {field:'price',title:'价格',width:100},
            {field:'d',title:'操作',width:100,
                formatter: function(value,row,index){
                    return "<a href='javascript:checks("+row.id+")'>取消</a>"
...
function checks(id) {
    ...
    $.post("HouseController/updateNotPass.do",{"id":id},function (data) {

        if(data.result>0){

            $("#dg").datagrid("reload");
        }else {
            $.messager.alert("error","取消审核失败");
        }
    },"json")
}
/*
    ...
    if($opts.length>=1){
        $.messager.confirm('删除操作', '确定需要删除吗？', function(r){
            if (r){
                ...
                $.post('type/deleteMoreType.do',{"id":ids},
                    function (data) {
                        if(data.result>=1){
