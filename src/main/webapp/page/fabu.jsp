...
<script>
      $(function () {
          $.post("proTypeController/getType.do",null,function (data) {
              for(var i=0;i<data.length;i++){
                  var chid=$("<option value='"+data[i].id+"'>"+data[i].name+"</option>");
                  $("#typeId").append(chid);
              }
          },"json");
              $.post("districtController/getDistrict.do",null,function (data) {
                  for(var i=0;i<data.length;i++){
                      var chid=$("<option value='"+data[i].id+"'>"+data[i].name+"</option>");
          ...
          $("#districtId").change(function () {
              ...
              $.post("proStreetController/getStreet.do",{"id":$("#districtId").val()},function (data) {
                  for(var i=0;i<data.length;i++){
                      var chid=$("<option value='"+data[i].id+"'>"+data[i].name+"</option>");
...
action="${pageContext.request.contextPath}/page/HouseController/insertHouse.do" enctype="multipart/form-data">
<DIV class=infos>
<TABLE class=field>
  <TBODY>
  <TR>
...
name=floorage></TD></TR>
  ...
  <TR>
      ...
      <TD><INPUT type="file" id="pfile" name="pfile" multiple="multiple"></TD></TR>
  </TBODY></TABLE>
